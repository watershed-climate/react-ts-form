...
type SchemaShape<SchemaType extends RTFSupportedZodTypes | ZodEffects<any, any>> = ReturnType<UnwrapEffects<SchemaType>['_def']["shape"]>
...
> = IndexOf<UnwrapMapping<Mapping>, readonly [
  IndexOfUnwrapZodType<SchemaShape<SchemaType>[key]>,
  any
]>;
...
key extends keyof z.infer<UnwrapEffects<SchemaType>>
> = 
IndexOfSchemaInMapping<Mapping, SchemaType, key> extends never ? never : Mapping[IndexOfSchemaInMapping<Mapping, SchemaType, key>];
...
> = [Level] extends [never] ? never : RequireKeysWithRequiredChildren<
 Partial<{
    [key in keyof z.infer<UnwrapEffects<SchemaType>>]:
    GetTupleFromMapping<Mapping, SchemaType, key> extends never 
    ? UnwrapEffects<SchemaType>["shape"][key] extends z.AnyZodObject 
      ?  PropType<Mapping, UnwrapEffects<SchemaType>["shape"][key], PropsMapType, Prev[Level]> 
      : UnwrapEffects<SchemaType>["shape"][key] extends z.ZodArray<any>  
        ? PropType<Mapping, UnwrapEffects<SchemaType>["shape"][key]['element'], PropsMapType, Prev[Level]>  
        : never 
    : GetTupleFromMapping<Mapping, SchemaType, key> extends readonly [any, any] // I guess this tells typescript it has a second element? errors without this check.
    ? Omit<
        ComponentProps<
        GetTupleFromMapping<Mapping, SchemaType, key>[1]
        >,
...
type RenderedFieldMap<SchemaType extends AnyZodObject | ZodEffects<any, any>> = {
  [key in keyof z.infer<UnwrapEffects<SchemaType>>]: UnwrapEffects<SchemaType>['shape'][key] extends z.AnyZodObject?
...
type RTFFormProps<Mapping extends FormComponentMapping, SchemaType extends z.AnyZodObject | ZodEffects<any, any>, PropsMapType extends PropsMapping = typeof defaultPropsMap,FormType extends FormComponent = "form"> = {
  /**
...
>(
  ...
  >({
    ...
    function renderComponentForSchemaDeep<NestedSchemaType extends RTFSupportedZodTypes | ZodEffects<any, any>, K extends keyof z.infer<UnwrapEffects<SchemaType>> >(
      _type: NestedSchemaType, props: PropType<Mapping, NestedSchemaType, PropsMapType> | undefined, key: K, prefixedKey: string, currentValue : any
    ): RenderedElement {
      ...
      if (!Component) {
        if (isAnyZodObject(type)) {
          ...
          return Object.entries(shape).reduce((accum, [subKey, subType]) => {
            accum[subKey] = renderComponentForSchemaDeep(subType, props && props[subKey] ? (props[subKey] as any) : undefined, subKey, `${prefixedKey}.${subKey}`, currentValue && currentValue[subKey]);
            return accum;
    ...
    function renderFields(schema: SchemaType, props: PropType<Mapping, SchemaType, PropsMapType> | undefined) {
      ...
      return Object.entries(shape).reduce((accum, [key, type]: [SchemaKey, RTFSupportedZodTypes]) => {
        ...
        accum[stringKey] = renderComponentForSchemaDeep(type, props as any, stringKey, stringKey, getValues()[key]);
        return accum;
      }, {} as RenderedObjectElements);
    }
    ...
    return (
      <FormProvider {..._form}>
        <ActualFormComponent {...formProps} onSubmit={() => {return submitFn()}} >
...
export function flattenRenderedElements(val: RenderedElement): JSX.Element[] {
  return Array.isArray(val) ?
    ...
    : (typeof val === 'object' && val !== null && !React.isValidElement(val)) ?
      Object.values(val).reduce((accum: JSX.Element[], val) => {return accum.concat(flattenRenderedElements(val as any))}, [] as JSX.Element[])
      : [val]
};
